...
public class InternalParameterCell extends ListCell<InternalParameter> { 
    ...
    @Override
    protected void updateItem(InternalParameter iParam, boolean empty) {
        super.updateItem(iParam, empty);

        if (empty || iParam == null) {
            setText(null);
            setGraphic(null);
        } else {
            setPadding(Insets.EMPTY);
            // Build the left column: A Label showing the dependency details.
            Label details = new Label(iParam.toString());
            ...
            editButton.setOnAction(e -> {
                if (internalUnitListener != null) {
                	internalUnitListener.onEdit(iParam);
                }
            });
            removeButton.setOnAction(e -> {
                if (internalUnitListener != null) {
                	internalUnitListener.onRemove(iParam);
                }
            });
    ...
    public interface InternalUnitListener {
        void onEdit(InternalParameter iParam);
        void onRemove(InternalParameter iParam);
    }
}
