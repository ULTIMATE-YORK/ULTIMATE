...
//            processBuilder.redirectErrorStream(true);
	...
	private String performPMC(Model model, String property) {
		...
		// originalModel.getHashParameters().values().stream().map(p -> {
		// try {
		// return String.format("%s (%s) : %s", p.getNameInModel(), p.getClass(),
		// p.getValue());
		// } catch (Exception e) {
		// return null;
		// }
		...
		// model.getModelId(), paramDebugString));
		try {
		...
		try {
			...
			if (modelFilePath != null && !modelFilePath.isEmpty()) {
				...
				// comments
				if (modelContent != null && (modelContent.contains("smg") || modelContent.contains("tsg")
						|| modelContent.contains("csg") || modelContent.contains("tptg"))) {
