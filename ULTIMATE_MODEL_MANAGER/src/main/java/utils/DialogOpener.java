...
import controllers.AddInternalController;
...
public class DialogOpener {
	 ...
	 */
	public static void openDialogWindow(Stage ownerStage, String path, String title, InternalParameter iParam) throws IOException {
	    FXMLLoader loader = new FXMLLoader(DialogOpener.class.getResource(path));
	    Parent root = loader.load();
	    EditInternalParameter controller = loader.getController();
	    controller.setInternalParameter(iParam);
	    // Create a new Stage
	    Stage dialogStage = new Stage();
	    // Set the owner of the dialog stage to the passed stage.
	    dialogStage.initOwner(ownerStage);
