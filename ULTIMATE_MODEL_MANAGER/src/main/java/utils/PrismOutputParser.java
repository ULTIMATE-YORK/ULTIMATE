...
import verification.PrismProcessAPI;
...
public class PrismOutputParser {
	...
	private static final Logger logger = LoggerFactory.getLogger(PrismOutputParser.class);
    ...
    public static Double getResult(String output) {
        ...
        if (matcher.find()) {
            try {
                ...
                logger.error("Failed to parse PRISM result as double: " + matcher.group(1));
                e.printStackTrace();
                return null;
            }
        }
        
        logger.error("No result found in PRISM output");
        return null;
    }
    ...
    public static String getRationalFunction(String output) {
        ...
        logger.warn("No rational function found in PRISM output");
        return null;
    }
